...
| [`open-drive.md`](./open-drive.md) | **The second strategy (#418):** a 10-minute opening-range breakout with a consolidation requirement, for the 09:30 open. **Specified and measured, not trading** — positive in R (+5.67R/13 trades) and unmonetisable at $500 *under the original selection rule* — #535 corrects that, see below — no individual rule separable from noise, and costly to the adaptive book if merged. Measured in `docs/reports/2026-08-02-the-0930-open-a-second-strategy.md`. **Selection refined the same day:** "first to trigger" proved an alphabetical lottery among same-bar fills landing on cap-crushed ~1%-risk picks; the banded sequential commit (rank by planned stop in `[3%, 10%)` at 09:40, one working order, roll on pre-fill stop breach) turns the same month from −0.5% into +6.0% at 4.2% dd — `docs/reports/2026-08-02-open-drive-picking-the-days-stock.md`. The spec carries the correction as of #535 (locked decision 7); the report also notes the ranking is lookahead-free only because every OD-5/5 setup is final at 09:40, which is not true of the bull-flag. |
| [`phase-2-roadmap.md`](./phase-2-roadmap.md) | The sequence from tracker to paper-live (epic #308). |
| `src/small_cap_stack/harvest/` (code, not a doc) | **The reconstructed-history producer (#431).** Rebuilds pre-market sessions from purchased vendor minute bars into `data/recon`, which the paper book publishes as `books_all` beside the live `books` (#430). Read `runner.py`'s module doc for the shape of the job and `reconstruct.py`'s for what a "scanner appearance" means when nobody sells the scanner's history. Operating it: `deploy/RUNBOOK.md` §13. |
...
### [`broker-costs.md`](./broker-costs.md) — IBKR cost model for a small UK cash account *(added 2026-07-15)*
What it costs to run the live strategy through IBKR from a **UK cash account at ~$500**. **IBKR Lite is US-residents-only**, so a UK client is on IBKR Pro and **Tiered vs Fixed is the only commission lever**. **Tiered wins** ($0.35 min vs $1.00) across ~$1.70–20 — ~$46/mo all-in vs ~$84/mo — with a crossover at ~150 shares (**$1.67/share** at a $250 position) below which Fixed is marginally cheaper. **The per-order minimum, not the per-share rate, is the whole story**: at 25-share orders you pay ~4× the headline rate, which is why drag is **9–13%/month at $500 but ~2.9% at $2,000**. Costs scale with order count and share count, *not* capital. Level 1 data = **$10/mo** (US Securities Snapshot & Futures Value Bundle), waived at ≥$30/mo commissions; **professional reclassification (~$100+/mo) is the tail risk that would end the experiment**. **A hard sizer constraint falls out of T+1 settlement**: total daily buy notional must not exceed the settled cash held at the start of the day — recycling the same $250 twice intraday is a good-faith violation. **No UK-accessible alternative exists** (Trading 212 / Freetrade / Lightyear lack pre-market and API; Webull UK has 04:00 ET pre-market but no retail API; Alpaca/Public are US-only). Verdict: **the cost structure is fine, the account size is the problem** — $500 is honest plumbing validation, not strategy validation.

### [`phase-2-roadmap.md`](./phase-2-roadmap.md) — the sequence from tracker to paper orders *(added 2026-07-17)*
Gates 0–7 from the Phase-1 tracker to paper trading (epic #308), and the state that motivates them: the **brain is built, the body is not**. Engine v2 *is* live and `portfolio/` selects/sizes/simulates exits over the captured dataset — but **nothing in the live runtime detects a setup** (`app.py` is scanner → capture → one EOD bar batch; the engine has zero live callers), **no order code exists**, and **the account's feed is ~15 min delayed** (`marketdata.py:4`). So `decisions.md` §D-21's "only simulate-exit gets swapped for place-order" is true of the sizing/selection brain but understates the gap — live detection, execution, and reconciliation are greenfield. **Pre-market is limit-only** (#37, confirmed from live trading), so the app fires every entry and exit itself, and the **exit-limit fill policy** — how far through the bid to price a stop exit, which can fail to fill in a fast drop — is the parameter that costs real money. The sleeper risk is **prefix stability**: the v2 detector segments the *longest valid* pole+consolidation over a day's bars, so live (growing prefix) and replay (full day) can disagree, which would silently invalidate the sim as a predictor of the live book — hence Gate 5 is log-only shadow detection *before* any order code.
...
### [`archive/review-workbench-spec.md`](./archive/review-workbench-spec.md) — the review workbench as proposed *(archived 2026-08-07, #542)*
The 2026-07-05 proposal for the chart-review page, **shipped as #140/#141** except Phase 2 item (5), the `review_compare.py` refine loop, which was never built. Kept as the record of what was intended, not of what exists — it predates the #288 cockpit rebuild and the #478 move of the chart into `docs/js/inspector.js`. It sat with **zero inbound references** for a month, which is how a plan quietly reads as a description; `tests/test_research_docs.py` now fails on an unreferenced doc.
...
1. **Float definition:** Is "float should be less than $20M" **20 million shares** or **$20M market value**? Changes the filter entirely. *(README §Strategy says "$20million"; needs your confirmation.)*
   > **✅ ANSWERED — shares, and it is not a filter.** `decisions.md` §D-01 locks 20 million **shares**.
...
3. **Exit strategy (README leaves this open):** Adopt tradepilot's proven scheme — fixed R:R target + breakeven auto-arm at 1.5R + high-water-mark trailing stop, most-protective-stop-wins? Or define a different TP/SL model? (Note tradepilot's ~10% bid-haircut on exits should be re-examined.)
   > **✅ ANSWERED 2026-07-16 (`decisions.md` §D-21/#476) — a different model.** The book exits on a
...
6. **Market-data entitlement:** The pre-market real-time requirement effectively forces an **IBKR Level-1 subscription** (≈ $10 L1 bundle, often waivable, + ~$4.50 streaming, + ~$1.50 NASDAQ TotalView non-pro). Commit to this paid-but-cheap data backbone, or attempt a free-only stack and accept delayed data (likely unworkable for 4am momentum)?
   > **✅ ANSWERED 2026-07-15** — see [`broker-costs.md`](./broker-costs.md) §4. **$10/mo** (US Securities Snapshot & Futures Value Bundle) covers the Level 1 need; no Level 2 required. Waived at ≥$30/mo commissions. **Tail risk: professional reclassification takes this to ~$100+/mo.**
...
- ~~**Pre-market 5-min bar completeness from IBKR** for low-volume names at 4am — sparse/missing bars would distort the bull-flag detection. Needs empirical validation.~~ **✅ Resolved 2026-06-29 (spike #9), and since confirmed by five weeks of live collection.** Active names get contiguous, gap-free 5-min bars from 04:00 ET; the only absence is a leading one, before the first trade. No interpolation needed. See [`decisions.md`](./decisions.md) §D-07 risk B.
- ~~**IBKR scanner adequacy** — whether its pre-market scan actually surfaces the strategy's candidates, or misses them. Unverified.~~ **✅ Largely resolved 2026-06-29 (spike #8), and superseded by production.** The API scanner returns a ranked pre-market candidate list, and it has been the live scanner since 2026-07-01. What *is* still open is narrower and better-posed: not "does it surface candidates" but **when** — #433 is measuring whether IBKR's change-percent reference price explains the appearance-time divergences the reconstruction can't otherwise account for. Note the 50-row cap named in §3 above has been measured as **never binding** (busiest live tick 45 symbols; pre-market peak 11, #460), so it is headroom, not a constraint. See [`decisions.md`](./decisions.md) §D-07 risk A.
...
- ~~**Actual IBKR market-data monthly cost** for this exact use case until subscriptions are selected in account management.~~ **✅ Resolved 2026-07-15** — $10/mo Level 1, waivable at $30/mo commissions; see [`broker-costs.md`](./broker-costs.md) §4. Residual unknowns (tiered minimum, exchange removal rate, waiver basis) are listed in that doc's §10 and need a Client Portal check.
