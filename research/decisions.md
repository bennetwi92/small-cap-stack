...
- **`setup_count` retired (issue #112, 2026-07-02):** the report keeps the `bull_flag` boolean but **drops the `setup_count` integer**. The count was un-gated (it counted flags across the whole segment window, including pre-appearance flags we could never take — unlike the appearance-gated R metrics, #99), never deduped by move or tied to outcome, and its only consumer was `bull_flag = setup_count > 0`; on its own it was noise. `bull_flag` is now derived from `RMetrics.setup_found` — exactly equivalent to `setup_count > 0` because every valid bull flag has strictly positive risk (`entry = breakout + entry_offset > breakout ≥ flag_low = stop`), so the setup the R-metrics pass already iterates *is* the flag. Option 3 (fold into `pump_index`/`pump_count`) was **deferred to #102**, which is still a research spike with no chosen move-start rule; the *meaningful* "which pump within the move" metric lands there. Decided before the 3-month collection so the report/`analysis` schema stays stable.
